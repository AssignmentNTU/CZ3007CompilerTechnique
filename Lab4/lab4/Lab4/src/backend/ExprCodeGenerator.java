...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitIntLiteral(IntLiteral nd) {
		...
		return IntConstant.v(nd.getEnd());
	}
	...
	public Value visitArrayIndex(ArrayIndex nd) {
		...
		//to get index
		Value indexAdded = wrap(nd.getIndex().accept(this));
		Value baseExpression = wrap(nd.getBase().accept(this));
//		units.add(Jimple.v().newArrayRef(baseExpression, indexAdded);
		return Jimple.v().newArrayRef(baseExpression, indexAdded);
	}
	...
	public Value visitNegExpr(NegExpr nd) {
		...
		return Jimple.v().newNegExpr(nd.accept(this));
	}
